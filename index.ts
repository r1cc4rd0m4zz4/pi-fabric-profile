...
function isFabricInstalled(): boolean {
  try {
    ...
    if (fs.existsSync(settingsPath)) {
      ...
      if (Array.isArray(settings.packages)) {
        return settings.packages.some(
          (p: string) => typeof p === "string" && p.includes("pi-fabric"),
        );
      }
    }
  } catch {
    // In caso di errore parsing o filesystem, non bloccare
  }
  return true;
}
  ...
  pi.registerCommand("fabric-profile", {
    ...
    handler: async (args: string, ctx: ExtensionCommandContext) => {
      ...
      if (!isFabricInstalled()) {
        ctx.ui.notify(
          "Avviso: 'pi-fabric' non risulta tra i pacchetti installati. Esegui 'pi install npm:pi-fabric' per abilitare l'esecuzione dei profili.",
          "warning",
        );
      }
